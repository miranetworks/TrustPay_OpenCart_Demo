...
class ControllerPaymenttrustpay extends Controller {
	public function index() {
		...
		$data['action'] = 'https://my.trustpay.biz/TrustPayWebClient/Transact';
		$data['vendor_id'] = $this->config->get('trustpay_vendor_id');
		$data['currency'] = $order_info['currency_code'];
		$data['amount'] = $this->currency->format($order_info['total'], $order_info['currency_code'],'', false);
		$data['txid'] = $this->session->data['order_id'];
		$data['appuser'] = $order_info['payment_firstname'] . ' ' . $order_info['payment_lastname'];
		$data['fail'] = $this->url->link('checkout/failure');
		$data['success'] = $this->url->link('payment/trustpay/waiting');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/trustpay.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/trustpay.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/trustpay.tpl', $data);
		}
